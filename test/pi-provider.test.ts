...
import { mergeModelWithPiCatalog, toPiCatalogSnapshot } from "../core/pi-model-metadata.ts";
...
test("provider models receive pricing and reasoning metadata dynamically from Pi catalog", () => {
	const piCatalog = {
		models: {
			"model-alpha": {
	...
	const snapshot = toPiCatalogSnapshot(piCatalog)!;
	...
	for (const models of [createCatalogModels(), createCatalogModels()]) {
		...
		const enriched = models.map((draft) => mergeModelWithPiCatalog(draft, snapshot).draft);
